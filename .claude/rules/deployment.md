...
**禁止**:
- hookファイルをコピーしただけで「設定完了」と報告する
- settings.jsonへの登録を確認せずに次の作業へ進む
- 設定ファイルを書き込んだだけで動作確認せずに「導入済み」と報告する

**必須手順**:
1. ファイルを配置する
2. settings.json等の設定ファイルで有効化されていることを確認する
3. 実際に1回実行して動作を確認する
4. 確認結果をもって完了報告する

### 5. deploy.sh -f（force）の不正使用

**deploy.sh -f はユーザーの明示的指示がある場合のみ使用する**

**禁止**:
- 新規ファイル追加に`-f`を使用する（`-f`なしで配置される）
- 「修正を反映したい」だけの理由で`-f`を使用する
- デプロイ対象を「〜だけ」「〜のみ」と根拠なく断定する

**理由**: `-f`はリポジトリ固有のカスタマイズを全て上書き破壊する。
デプロイ対象は機械的検索（`grep -r`, `find`）で網羅的に調査すること。
