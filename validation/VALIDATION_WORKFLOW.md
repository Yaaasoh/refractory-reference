...
1. **機密情報の除去**: 特定のクライアント・プロジェクトを識別できる情報がないこと
2. **用語集としての適切性**: 仕様値・カタログ情報ではなく、用語定義であること
3. **データ品質**: 文字化け・構造崩れがないこと
...
| サブカテゴリ | 説明 | 検索方法 |
|-------------|------|---------|
| A1. 企業名 | 耐火物メーカー、製鉄会社、下請会社 | パターンファイル参照 |
| A2. 人名 | 一般的な日本人苗字 | パターンファイル参照 |
| A3. 敬称・識別子 | 「様」「殿」「御中」、内部識別子 | 正規表現検索 |
...
| `patterns/identifiers.txt` | 識別子・敬称・機密マーカー | 約20パターン |

**設計思想**:
- 業界で一般的な企業名・苗字を網羅的にリスト化
- 特定クライアントを識別しない一般化されたパターン
- パターンへの該当 = 機密情報の可能性 → 要確認
